...
#when you insert, it goes to the bottom of the list, then it checks if the parent is more, if it is that number stays there
def heapify_up(heap: MinHeap, index: int) -> MinHeap:
    if index == 0:
        return heap  # Base case: reached the root of the heap
    parent_index = (index - 1) // 2 #the parent
    heap_data = heap.data
